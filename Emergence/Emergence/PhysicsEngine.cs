    ...
    public class PhysicsEngine {
        ...
        class PushOutComparer : IComparer<Vector3> {
            ...
            int IComparer<Vector3>.Compare(Vector3 a, Vector3 b) {
                float aSize = Vector3.Dot(a, a) * (1-Math.Abs(Vector3.Dot(Vector3.Normalize(a), move))),
                      bSize = Vector3.Dot(b, b) * (1-Math.Abs(Vector3.Dot(Vector3.Normalize(b), move)));
                if (aSize > bSize)
                    return -1;
                else if (aSize < bSize)
                    return 1;
        ...
        public Vector3 getPushOut(BoundingBox mover, Brush brush, Vector3 move) {
            ...
            foreach (Face f in brush.faces) {
                ...
                foreach(LineSegment l in segments)  {
                     ...
                     * of the line and the face's plane. If t <= 1 then the point is on the line segment
                    ...
                    if (Math.Abs(denom) <= float.Epsilon)   continue;
                    float t = -(d + Vector3.Dot(norm, l.a)) / denom;
                    //if t < 1 then this line segment collides with our plane
                    if (t < 1) {
                        ...
                        if (inFace) {
                             ...
                             */
                            Vector3 pointBehind = (Vector3.Dot(f.getCenter(), l.a) > 0 ? l.a : l.b);
                            /* The direction vector of the shortedt line from pointBehind to the face
